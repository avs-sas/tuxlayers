...
from tuxlayers import (Node, LayerDAG, validate_path, parse_path_configs_and_add_dummy_nodes,
                       load_layer_config, cli, parse_dag_from_layers)
...
def test_layer_dag_show_failure(caplog):
    dag = LayerDAG()
    dag.create_node("root")
    with patch("tuxlayers.TopologicalSorter", side_effect=Exception("TopologicalSorter failed")):
...
def test_cli_help():
    ...
    assert "tuxlayers is used to maintain a hiearchy" in result.output
...
def test_parse_dag_from_layers_success(tmp_path):
    ...
    ctx = MagicMock()
    ctx.obj = {"LAYER_SOURCE": str(layers_dir), "PATHS_SOURCE": None}
...
def test_cli_full_run(tmp_path):
    ...
    runner = CliRunner()
    # Running info command which is a sub-command of cli
    result = runner.invoke(cli, ["--layersdir", str(layers_dir), "info"])
    assert result.exit_code == 0
...
def test_parse_dag_from_layers_with_paths(tmp_path):
    ...
    ctx = MagicMock()
    ctx.obj = {"LAYER_SOURCE": str(layers_dir), "PATHS_SOURCE": str(paths_dir)}
...
def test_cli_no_layers(tmp_path):
    runner = CliRunner()
    # Use empty dir instead of nonexistent to test line 187
    layers_dir = tmp_path / "empty"
    layers_dir.mkdir()
    # Wait, cli function checks if layersdir is passed.
    # If I pass --layersdir "", it might trigger the 'else' block.
    result = runner.invoke(cli, ["--layersdir", "", "info"])
    assert "No layers selected. Continuing without them." in result.output

def test_parse_dag_from_layers_invalid_dir():
    ctx = MagicMock()
    ctx.obj = {"LAYER_SOURCE": "/nonexistent_dir_random_name_123", "PATHS_SOURCE": None}
    with pytest.raises(SystemExit):
        parse_dag_from_layers(ctx)
